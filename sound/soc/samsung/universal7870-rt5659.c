...
void universal7870_set_mclk(int enable)
{
	...
	if (priv->mclk) {
		if (enable) {
			...
			mclk_usecount++;
			dev_dbg(card->dev, "mclk enabled: cnt %d\n", mclk_usecount);
		} else {
			clk_disable(priv->mclk);
			mclk_usecount--;
			dev_dbg(card->dev, "mclk disabled: cnt %d\n", mclk_usecount);
		}
	} else {
		dev_dbg(card->dev, "%s: %d usecount %d\n", __func__, enable, mclk_usecount);
		...
		if (enable) {
			if (mclk_usecount == 0) {
				dev_dbg(card->dev, "%s: mclk_enable\n", __func__);
			...
			if (mclk_usecount == 0) {
				dev_dbg(card->dev, "%s: mclk_disable\n", __func__);
				regmap_update_bits(pmureg,
						EXYNOS_PMU_PMU_DEBUG_OFFSET,
						0x1F01, 0x1F01);
			}
